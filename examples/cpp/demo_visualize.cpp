...
std::vector<Point> parsePointCloud(const std::string& filename) {
...
void read_pcd(std::string path, Eigen::MatrixXf& cloud) {
  auto res = parsePointCloud(path);
  cloud.resize(res.size(), 4);
  for (int i=0; i<res.size(); i++)
  {
    cloud.row(i) << res[i].x, res[i].y, res[i].z, res[i].intensity / 255.0;
  }  
}
...
int main(int argc, char* argv[]) {
  ...
  if(input_cloud_filepath.find(".bin") != std::string::npos)
    read_bin(input_cloud_filepath, cloud);
  else if(input_cloud_filepath.find(".pcd") != std::string::npos)
